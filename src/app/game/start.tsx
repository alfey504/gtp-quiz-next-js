        ...
        <div className="mt-28 flex flex-col items-center">
            <form className='flex flex-col items-center'>
                ...
                <div className='mt-10 flex flex-row w-1/2 justify-between'>
                    <SelectButton className='w-96 h-12' name='EASY' selected={props.difficulty} setSelected={props.setDifficulty}/>
                    <SelectButton className='w-96 h-12' name='MEDIUM' selected={props.difficulty} setSelected={props.setDifficulty}/>
                    <SelectButton className='w-96 h-12' name='HARD' selected={props.difficulty} setSelected={props.setDifficulty}/>
                </div>
...
export function SelectButton(props: {name: string, selected: string, className:string,setSelected: (selected: string)=>void}): ReactNode{
    if(props.selected === props.name){
        return <button type="button" className={'border border-green bg-green text-white p-2 pr-3 pl-3 text-center mr-5 ml-5 rounded-3xl shadow-lg ' + props.className} onClick={() => props.setSelected(props.name)}>{props.name}</button>  
    }else{
        return <button type="button" className={'border border-white text-white p-2 pr-3 pl-3 text-center mr-5 ml-5 rounded-3xl hover:bg-white hover:text-black ' + props.className} onClick={() => props.setSelected(props.name)}>{props.name}</button>
    }
}

export function CustomButton(props: {className: string, title:string, onClick: ()=> void}){
    const {className, title, onClick} = props
    return <div className={className}><button type="button" className="border border-green text-white p-2 pr-3 pl-3 w-96 h-12 text-center bg-green rounded-xl hover-bg-light-green" onClick={onClick}>{title}</button></div>
}